...
<script type="text/javascript">
	...
	// function reloadList {
	// 	document.getElementById('nameList').reload(true);
	// 	document.getElementById('nameList').contentWindow.location.reload(true);
	// }
</script>
		...
		<div class="activityList">
			...
			<form class="activityList_insert"
				...
				<fieldset>
					...
					<div class="activity_filling">
						<select name="name" id="nameList" style="height: 35px;">
							<c:forEach items="${sessionScope.FriendBean}" var="category">
								<%-- 		                <option value="${category.friendNo}"  --%>
								<%-- 		                    <c:if test="${category.friendNo eq selectedCatId}">selected="selected"</c:if> --%>
								<!-- 		                    > -->
								<%-- 		                    ${category.friendName}，${category.friendPhone} --%>
								<!-- 		                </option> -->
				...
				<table
					...
					<tbody>
						...
						<c:forEach varStatus="stVar" var="teammateBean"
							items="${sessionScope.TeammateBean}">
							<tr>
								...
								<td>
									...
									<form
										action="MatesDelete.do?activityNo=${sessionScope.activityNo}&teammateNo=${teammateBean.teammateNo}"
										method="post">
