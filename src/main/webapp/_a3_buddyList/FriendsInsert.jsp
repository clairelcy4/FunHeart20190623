		...
		<div class="friendList">
			...
			<div>
				<table
					...
					<tbody>
						<c:forEach varStatus="stVar" var="mateBean"
							items="${products_DPP}">
							<tr>
								<td scope="row">${stVar.index+1}</td>
								<th>${mateBean.friendName}</th>
								<td>${mateBean.friendPhone}</td>
								<td>
									<form action="FriendsDelete.do?friendNo=${mateBean.friendNo}"
										method="post">
										<button type="submit" class="goToSublist"
											onClick="return confirm('確定要刪除：${mateBean.friendName}，電話：${mateBean.friendPhone}&nbsp;?\r此動作將無法復原！');">刪除</button>
									</form>
								</td>
							</tr>
						</c:forEach>
